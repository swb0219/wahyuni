...
// Dirubah 
const pengirim = "Ais";
const nomorWa = "6282291476572" // awalan nomor 0 harus di awalin 62
const textWa = "kangen ngn sayang"
const pesan = `Meski kta bukan orang pertama, kedua, dan ketiga di ngn p hati, kta harap bisa menjadi yang terakhir dalam ngn p hati saat ini.
Kta p Pundak mungkin tra selalu ada saat ngn bersedih dg manangis, tapi kta p cinta akan selalu ada untuk ngn selamanya. I LOVE YOU Sayang`;
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'kangen kita k trd sayang?',
    ...
    preConfirm: () => {
      ...
      if (!login) {
        Swal.showValidationMessage(`kangen kita k trd sayang?' :(`)
      }
      return { login }
    }
  }).then((result) => {
    ...
    let txt = `Asli tu? ${result.value.login}, kita juga kangen skali ${namaAwal.toUpperCase()}, ${pesan}`;
